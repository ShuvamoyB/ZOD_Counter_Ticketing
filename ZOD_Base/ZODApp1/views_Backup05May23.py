...
def index(request):
    ...
    return render(request, 'index.html', {'startdt':StartDate, 'starttime':StartTime, 'starttimestr':starttimestr})
...
def activation(request):
    ...
    LastEventID320 = Xamp320EventwiseJsondataReceived.objects.only('eventid').get(pk=LastId_320).eventid
    ...
    for j in range(1, (LastEventID320+1)):
        EventID = Xamp320EventwiseJsondataReceived.objects.only('eventid').get(pk=j).eventid
        print("Event ID: ",EventID)
        EventCode = Xamp320EventwiseJsondataReceived.objects.only('eventid').get(pk=j).eventcode
        print("Event Code: ",EventCode)
        TicketPrefix = Xamp320EventwiseJsondataReceived.objects.only('eventid').get(pk=j).ticketprefix
        print("Event Code: ",TicketPrefix)
        EventName = Xamp320EventwiseJsondataReceived.objects.only('eventid').get(pk=j).eventname
        print("Event Name: ",EventName)
        AdultRate = Xamp320EventwiseJsondataReceived.objects.only('eventid').get(pk=j).adult
        print("Adult Rate: ",AdultRate)
        ChildRate = Xamp320EventwiseJsondataReceived.objects.only('eventid').get(pk=j).child
        print("Child Rate: ",ChildRate)
        VideoPerUnitRate = Xamp320EventwiseJsondataReceived.objects.only('eventid').get(pk=j).perunit
...
def generate_ticket_pc(request):
    ...
    generated_ticket_no = random.randint(42187, 8765643)
    ...
    TypeForCounter='C'
    TicketSrNo=str(Xamp340TicketSoldDetails.objects.only('ticketlast_serial_no_x340').latest('ticketlast_serial_no_x340').ticketlast_serial_no_x340)
    print("TicketSrNo: ",TicketSrNo)
    EventCode=str(Xamp340TicketSoldDetails.objects.only('event_code_from_t520_x340').latest('event_code_from_t520_x340').event_code_from_t520_x340)
    ...
    if request.method == "POST":
        ...
        for j in range(1, (LastEventID320+1)):
            ...
            modelTable7.ticket_no_x340 = generated_ticket_no
        ...
        # if EntryTotalNoOfAdults==True and EntryTotalNoOfChild==False and TotalVideoPerUnit==False and AquariumTotalNoOfAdults==False:
        #     return redirect('print_ticket')
        # elif EntryTotalNoOfChild>1:
        #     return redirect('print_ticket')
        # elif TotalVideoPerUnit>1:
        #     return redirect('print_ticket')
        # else:
        #     pass
...
def statement(request):
    DateRange=0
    DataFrm_x300 = Xamp300Log.objects.all()
    DataFrm_x340 = Xamp340TicketSoldDetails.objects.all()
    ...
    print('ZooOpenTime: ',ZooOpenTime)
    print('ZooCloseTime: ',ZooCloseTime)

    if request.method == "POST":
        DateRange = request.POST.get('date_range')
        print("\nDate Range: ", DateRange)

        SelectedDateFrm = DateRange[0:10]
        SelectedDateTo = DateRange[13:23]

        print(SelectedDateFrm[::-1])
        print(SelectedDateTo[::-1])
        
        SelectedDateFrm = '2023-05-05'
        SelectedDateTo = '2023-05-06'
        
        DateFrm = SelectedDateFrm+' '+ZooOpenTime
        DateTo = SelectedDateTo+' '+ZooCloseTime
        ...
        context = {'daterange':DateRange,'selecteddata_x340':SelectedData_x340}
